...
{
  $range_diff = $show_diff['date_diff'] * 0 + 0.5;
}
...
//หาการลาครั้งล่าสุด
$sql_latest = "SELECT * FROM qry_leavedoc WHERE userid = '".$row_show['userid']."' ORDER BY docid DESC LIMIT 1,1";
...
<?php if ($row_show["t_subject"] != ""): ?> ครูพิเศษสอนวิชา <?echo $row_show['t_subject']; ?> ชั้น <?echo $row_show['t_sclass']; ?><?php endif ?> ขอ<?php if ($row_show["category"] == 1): ?>ลากิจ<?php elseif ($row_show["category"] == 2): ?>ลาป่วย<?php elseif ($row_show["category"] == 3): ?>ลาคลอด<?php else: ?>มาสาย<?php endif ?> เนื่องจาก <?echo $row_show['reason']; ?>
 ...
 <? if ($count_latest == 0): ?><? else : ?>ครั้งสุดท้ายข้าพเจ้าได้<?php if ($show_latest["category"] == 1): ?>ลากิจ<?php elseif ($show_latest["category"] == 2): ?>ลาป่วย<?php elseif ($show_latest["category"] == 3): ?>ลาคลอด<?php else: ?>มาสาย<?php endif ?> 
 ตั้งแต่วันที่ <? echo DateThai($show_latest["start_date"]); ?> (<? if($show_latest["s_time_range"] == 1):?>เต็มวัน<? elseif($show_latest["s_time_range"] == 2):?>ครึ่งวันเช้า<?php else: ?>ครึ่งวันบ่าย<? endif ?>) ถึงวันที่ <? echo DateThai($show_latest["end_date"]); ?> (<? if($show_latest["e_time_range"] == 1):?>เต็มวัน<? elseif($show_latest["e_time_range"] == 2):?>ครึ่งวันเช้า<?php else: ?>ครึ่งวันบ่าย<? endif ?>)<? endif ?>
</p>
